    ...
    private void selectClosestResultBasedOnTextFieldValue(boolean affect, boolean inFocus) {
        ...
        for (int i=0; i<items.size(); i++) {
            if (AutoCompleteComboBoxListener.this.comboBox.getEditor().getText().toLowerCase().equals(items.get(i).toString().toLowerCase())) {
                try {
                	@SuppressWarnings("rawtypes")
					ListView<String> lv = ((ComboBoxListViewSkin) AutoCompleteComboBoxListener.this.comboBox.getSkin()).getListView();
        ...
        if (!inFocus && comboBox.getEditor().getText() != null && comboBox.getEditor().getText().trim().length() > 0) {
            ...
            //KeyEvent ke = KeyEvent.impl_keyEvent(comboBox, KeyCode.ENTER.toString(), KeyCode.ENTER.getName(), KeyCode.ENTER, false, false, false, false, KeyEvent.KEY_RELEASED);
           
            KeyEvent ke = new KeyEvent(null,comboBox,KeyEvent.KEY_RELEASED,KeyCode.ENTER.toString(), KeyCode.ENTER.getName(), KeyCode.ENTER,false, false, false, false);

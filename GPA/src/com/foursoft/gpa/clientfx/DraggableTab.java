...
public class DraggableTab extends Tab {
    ...
    static {
        ...
        Rectangle dummy = new Rectangle(3, 20, Color.web("#00F300"));
    ...
    public DraggableTab(String text, Node icon) {
    	init(text,icon);
    }
    

	private void init(String text, Node icon) {
		tabTitel = new HBox(icon,new Label(text));
		tabTitel.setSpacing(2);
        setGraphic(tabTitel);
        detachable = true;
        dragStage = new Stage();
        ...
        tabTitel.setOnMouseDragged(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent t) {
                dragStage.setWidth(tabTitel.getWidth() + 10);
                dragStage.setHeight(tabTitel.getHeight() + 10);
        ...
        tabTitel.setOnMouseReleased(new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent t) {
                ...
                if(!t.isStillSincePress()) {
                    ...
                    if(!detachable) {
                    	/*
                    	SplitPane splitTab=new SplitPane();
                        splitTab.setDividerPositions(0,5);
                        splitTab.setOrientation(Orientation.VERTICAL);
                        AnchorPane parent=(AnchorPane) oldTabPane.getParent();
                        splitTab.getItems().add(oldTabPane);                     
                        parent.getChildren().add(splitTab);
                        */
                        return;
                    }
    ...
    private Rectangle2D getAbsoluteRect(HBox node) {
        return new Rectangle2D(node.localToScene(node.getLayoutBounds().getMinX(), node.getLayoutBounds().getMinY()).getX() + node.getScene().getWindow().getX(),
                node.localToScene(node.getLayoutBounds().getMinX(), node.getLayoutBounds().getMinY()).getY() + node.getScene().getWindow().getY(),
                node.getWidth(),
                node.getHeight());
    ...
    private Rectangle2D getAbsoluteRect(Tab tab) {
    	...
    	DraggableTab dragTab=(DraggableTab) tab;
        return getAbsoluteRect(dragTab.getHbox());
    }
